...
class PCGradOptimizerHook(OptimizerHook):
    ...
    def after_train_iter(self, runner):
        ...
        # Auxiliary losses (e.g., dense depth) are backpropagated normally,
        # outside PCGrad projection and excluded from conflict stats.
        ...
        for i, task in enumerate(tasks):
            ...
            # Keep graph alive if more task backwards or aux backward remain.
        ...
        # ---- Step 1b: Auxiliary backward (excluded from PCGrad) ----
        # Zero shared grads again so we can isolate aux-only contribution on
        # shared params; non-shared params keep their accumulated task grads
        # and aux backward will add its own contribution on top.
        aux_shared_grads = {}
        if has_aux:
            for params in self._param_groups.values():
                for p in params:
            ...
            aux_loss.backward()
            for gk, params in self._param_groups.items():
                aux_shared_grads[gk] = self._flatten_grads(params).clone()
        ...
        for gk, params in self._param_groups.items():
            ...
            # Mix in auxiliary (non-PCGrad) shared grads, e.g. dense depth.
            if gk in aux_shared_grads:
                merged = merged + aux_shared_grads[gk]
