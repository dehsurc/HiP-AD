...
class NuScenes3DDataset(Dataset):
    ...
    def get_data_info(self, index):
        ...
        # Load map teacher cache (MapTR polylines).
        # We .copy() teacher_map_pts so any in-place transform in the pipeline
    ...
    def get_ann_info(self, index):
        ...
        if 'gt_ego_fut_trajs' in info:
            ...
            # HiP-AD planning heads expect *_2hz keys; nuScenes is 2Hz by default.
            anns_results['gt_ego_fut_trajs_2hz'] = info['gt_ego_fut_trajs']
            anns_results['gt_ego_fut_masks_2hz'] = info['gt_ego_fut_masks']
    ...
    def evaluate(
        ...
        res_path = osp.join(work_dir, res_path)
        ...
        if eval_mode['with_map']:
            ...
            result_path = self.format_map_results(results, prefix=work_dir)
        ...
        if eval_mode['with_motion']:
            ...
            result_files = self.format_motion_results(results, jsonfile_prefix=work_dir, thresh=thresh)
            motion_results_dict = self._evaluate_single_motion(result_files, work_dir, logger=logger)
            results_dict.update(motion_results_dict)
        
        if eval_mode['with_planning']:
            from .evaluation_nuscenes.planning.planning_eval import planning_eval
