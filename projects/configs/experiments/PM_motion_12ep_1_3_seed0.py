"""PM_motion: motion-only training on top of frozen det.

Motion query is generated inside refine from det_instance_feature, so the det
modules (instance_bank, anchor_encoder, deformable, refine) must run in
forward. We keep them in the model but freeze them (paramwise lr_mult=0 +
norm_eval=True on backbone). Only motion_anchor_encoder and motion_refine are
trained.

task_select = ["det", "motion"]   # det runs forward (loss weight=0), motion gets loss
query_select = ["det"]            # motion is generated from det, not a separate bank

Loads PM_map checkpoint to inherit:
  - img_backbone / img_neck / depth_branch (originally trained in PM_det)
  - det_* modules (originally trained in PM_det; PM_map didn't have them so
    they would be missing from PM_map ckpt — see note below)

NOTE on load_from: PM_map's checkpoint does NOT contain det_* parameters
(because PM_map's model has no det modules). So loading from PM_map alone
will leave det_* freshly initialized. To inherit the trained det weights you
must either:
  (a) chain load: PM_det → PM_motion directly (current default below), OR
  (b) merge PM_det + PM_map ckpts via a small script and load the merged file.
For now we use (a) — load_from = PM_det. The shared encoder (backbone) is
identical between PM_det and PM_map only if PM_map froze it (which it does in
this experiment design), so PM_det's ckpt is the right source for det_ + the
frozen encoder.
"""
log_level = "INFO"
...
work_dir = "work_dirs/exp/PM_motion_12ep_1_3_seed0"
...
wandb_project = "hipad"
wandb_name = "PM_motion_12ep_1_3_seed0"
log_config = dict(
    interval=50,
    hooks=[
...
# Load from PM_det (which has trained backbone + det_* + decoder ops).
load_from = "./work_dirs/exp/PM_det_12ep_1_3_seed0/latest.pth"
import os as _os
_resume = "./work_dirs/exp/PM_motion_12ep_1_3_seed0/latest.pth"
...
# motion / planning shape constants (motion uses fut_ts, fut_mode)
fut_ts = 12
fut_mode = 6
ego_fut_ts = 6
ego_fut_cmd = 3
...
# ====== PM_motion: motion trained, det frozen ======
task_config = dict(with_onedecoder=True)
task_select = ["det", "motion"]   # det forward needed; det loss zeroed below
query_select = ["det"]            # motion query is generated from det inside refine
...
model = dict(
    ...
    head=dict(
        ...
        onedecoder_head=dict(
            ...
            # ----- det modules (kept for forward; frozen via paramwise) -----
            ...
            # ----- operation (det-only attention, matches PM_det) -----
            ...
            # ----- loss: motion only (det losses zeroed; det forward kept for motion) -----
            # det_gt_loss_weight=0 ensures det cls/reg losses are zeroed in loss_det.
            det_gt_loss_weight=0.0,
            loss_det_cls=dict(type="FocalLoss", use_sigmoid=True, gamma=2.0, alpha=0.25, loss_weight=0.0),
            loss_det_reg=dict(
...
# ================== training ========================
# Freeze everything except motion_anchor_encoder and motion_refine.
# mmcv paramwise_cfg.custom_keys uses substring match against param name.
# Pattern "head.onedecoder_head.layers" matches all decoder operation layers
# (graph_model, temp_graph_model, ffn, norm, custom_op instances stored in
# the nn.ModuleList named 'layers' in SparseOneDecoder).
optimizer = dict(
    type="AdamW",
    lr=1e-4,
    weight_decay=0.001,
    paramwise_cfg=dict(
        custom_keys={
            ...
            "head.onedecoder_head.det_": dict(lr_mult=0.0),
            "head.onedecoder_head.fc_before": dict(lr_mult=0.0),
            "head.onedecoder_head.fc_after": dict(lr_mult=0.0),
            "head.onedecoder_head.layers": dict(lr_mult=0.0),
            # motion_anchor_encoder, motion_refine: no override → full lr=1e-4
        }
    ),
)
...
# ================== eval ========================
eval_mode = dict(
    with_det=True,           # need det results for motion eval pipeline
    with_tracking=False,
    with_map=False,
    with_motion=True,
    with_planning=False,
