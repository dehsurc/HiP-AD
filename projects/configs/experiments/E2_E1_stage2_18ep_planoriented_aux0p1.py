...
work_dir = "work_dirs/exp/E2_E1_stage2_18ep_planoriented_aux0p1"
...
wandb_name = "E2_E1_stage2_18ep_planoriented_aux0p1"
log_config = dict(
    interval=50,
    hooks=[
...
load_from = "/home/yongjae/e2e/HiP-AD/ckpts/rev_nusc/70ep.pth"
resume_from = None
workflow = [("train", 1)]
...
model = dict(
    ...
    head=dict(
        ...
        onedecoder_head=dict(
            ...
            # loss
            # >>> planning-oriented: aux(det/map/motion) x0.1, plan/ego/depth kept at baseline <<<
            loss_det_cls=dict(type="FocalLoss", use_sigmoid=True, gamma=2.0, alpha=0.25, loss_weight=0.2),  # 2.0 -> 0.2
            loss_det_reg=dict(type="SparseBox3DLoss",
                              loss_box=dict(type="L1Loss", loss_weight=0.025),  # 0.25 -> 0.025
                              loss_centerness=dict(type="CrossEntropyLoss", use_sigmoid=True, loss_weight=0.1),  # 1.0 -> 0.1
                              loss_yawness=dict(type="GaussianFocalLoss", loss_weight=0.1),  # 1.0 -> 0.1
                              cls_allow_reverse=[det_class_names.index("barrier")]),
            loss_map_cls=dict(type="FocalLoss", use_sigmoid=True, gamma=2.0, alpha=0.25, loss_weight=0.1),  # 1.0 -> 0.1
            loss_map_reg=dict(type="SparseLineLoss",
                              loss_line=dict(type="LinesL1Loss", loss_weight=1.0, beta=0.01),  # 10.0 -> 1.0
                              num_sample=map_num_pts,
                              roi_size=map_roi_size),
            loss_ego_status=dict(type="L1Loss", loss_weight=1.0),  # kept
            loss_plan_cls=dict(type="FocalLoss", use_sigmoid=True, gamma=2.0, alpha=0.25, loss_weight=0.5),  # kept
            loss_plan_reg=dict(type="L1Loss", loss_weight=1.0),  # kept
            loss_motion_cls=dict(type="FocalLoss", use_sigmoid=True, gamma=2.0, alpha=0.25, loss_weight=0.02),  # 0.2 -> 0.02
            loss_motion_reg=dict(type="L1Loss", loss_weight=0.02),  # 0.2 -> 0.02
            # weights
