...
experiment_name = 'E13_test_fd_fullgrad'
work_dir = 'work_dirs/exp/E13_test_fd_fullgrad'
wandb_entity = 'e2ekd'
wandb_project = 'hipad'
num_gpus = 2
batch_size = 8
...
log_config = dict(
    ...
    hooks=[
        dict(
            ...
            by_epoch=False,
            sci_keys=['map_loss_kd_feat', 'map_loss_kd_spatial'],
            sci_prefixes=['map_loss_kd_feat_', 'map_loss_kd_spatial_']),
        dict(
            type='WandbLoggerHook',
            init_kwargs=dict(
                entity='e2ekd',
                project='hipad',
                name='E12_stage1_spatial_mapadapter_decay_a2to05_l5a05'),
            by_epoch=False)
    ])
load_from = None
resume_from = None
...
selected_tasks = ['det', 'map', 'plan', 'ego']
model_embed_dims = 256
num_decoder_layers = 6
num_single_frame_decoder = 1
plan_num_future_steps = 6
plan_num_commands = 3
plan_num_modes = 6
plan_anchor_type = ('temp', '2hz')
plan_anchor_path = 'data/kmeans/kmeans_plan_6.npy'
motion_num_future_steps = 12
motion_num_modes = 6
det_class_names = [
...
train_ann_file = 'data/infos/nuscenes_infos_train.pkl'
val_ann_file = 'data/infos/nuscenes_infos_val.pkl'
input_modality = dict(
    use_lidar=False,
    use_camera=True,
    use_radar=False,
...
map_roi_size = (30, 60)
map_num_points = 20
object_range_threshold = 55
image_norm_mean = [123.675, 116.28, 103.53]
image_norm_std = [58.395, 57.12, 57.375]
image_norm_to_rgb = True
resize_lim = (0.4, 0.47)
final_dim = (256, 704)
bot_pct_lim = (0.0, 0.0)
image_rot_lim = (-5.4, 5.4)
source_image_size = (900, 1600)
random_flip = True
rot3d_range = [0, 0]
...
min_lr_ratio = 0.001
dense_depth_loss_weight = 0.2
cls_threshold_to_reg = 0.05
focal_gamma = 2.0
focal_alpha = 0.25
det_cls_loss_weight = 2.0
det_box_loss_weight = 0.25
map_cls_loss_weight = 1.0
map_line_loss_weight = 10.0
map_line_loss_beta = 0.01
map_match_cls_cost_weight = 1.0
map_match_line_cost_weight = 10.0
map_match_line_cost_beta = 0.01
ego_status_loss_weight = 0.0
plan_cls_loss_weight = 0.0
plan_reg_loss_weight = 0.0
motion_cls_loss_weight = 0.2
motion_reg_loss_weight = 0.2
det_task_weight = 0.85
map_task_weight = 1.75
map_teacher_cache_path = 'data/cache/map/maptrv2_teacher_train_feat_top20_l345.pkl'
map_teacher_cached_layers = (3, 4, 5)
map_teacher_feature_dim = 256
map_teacher_num_queries = 20
map_teacher_require_features = True
map_feature_distill_start_alpha = 2.0
map_feature_distill_end_alpha = 1.0
map_feature_distill_decay_start_epoch = 8
map_feature_distill_cfg = dict(
    map_feature_match_mode='gt_anchor',
    map_feature_distill_alpha=2.0,
    map_feature_distill_score_thr=0.3,
    map_feature_distill_dist_thr=4.0,
...
decoder_layer_ops = [
    'gnn', 'inter_gnn', 'norm', 'split', 'deformable', 'concat', 'ffn', 'norm',
    'split', 'refine'
]
temporal_decoder_layer_ops = [
...
flash_attention_256_cfg = dict(
    type='MultiheadFlashAttention',
    embed_dims=256,
    num_heads=8,
    batch_first=True,
    dropout=0.1)
flash_attention_512_cfg = dict(
    type='MultiheadFlashAttention',
    embed_dims=512,
    num_heads=8,
    batch_first=True,
    dropout=0.1)
deformable_attention_cfg = dict(
    embed_dims=256,
    num_groups=8,
    num_levels=4,
    num_cams=6,
...
model = dict(
    ...
    head=dict(
        ...
        onedecoder_head=dict(
            ...
            det_task_weight=0.85,
            map_task_weight=1.75,
            task_grad_monitor_interval=0,
            task_grad_monitor_mode='full_task_flow',
            map_feature_match_mode='gt_anchor',
            ...
            temp_graph_model=dict(
                ...
                query_select=['det', 'map', 'plan', 'ego'],
                query_list=[['det'], ['map'], ['plan', 'ego'], ['plan',
                                                                'ego']],
            ...
            map_spatial_use_map_adapter=True,
            map_spatial_adapter_hidden_dim=64,
            map_spatial_detach_fpn_for_kd=False)))
dataset_common = dict(
    ...
    work_dir=
    'work_dirs/exp/E12_stage1_spatial_mapadapter_decay_a2to05_l5a05'
)
img_norm_cfg = dict(
...
train_collect_keys = [
    ...
    'gt_agent_fut_trajs', 'gt_agent_fut_masks', 'gt_ego_fut_trajs',
    'gt_ego_fut_masks', 'gt_ego_fut_cmd', 'gt_ego_fut_trajs_2hz',
    'gt_ego_fut_masks_2hz', 'ego_status', 'ego_status_mask',
    'teacher_map_logits', 'teacher_map_pts', 'teacher_map_scores',
    'teacher_map_features', 'teacher_map_spatial_pts',
    'teacher_map_spatial_labels', 'teacher_map_spatial_features'
]
train_pipeline = [
    ...
    dict(
        ...
        keys=[
            ...
            'teacher_map_spatial_pts', 'teacher_map_spatial_labels',
            'teacher_map_spatial_features'
        ],
...
train_dataset = dict(
    ...
    'data/cache/map/maptrv2_teacher_train_feat_top20_l345.pkl',
...
data = dict(
    ...
    train=dict(
        ...
        map_teacher_feature_dim=256,
        map_teacher_require_features=True,
        map_spatial_teacher_cache_path=
        'data/cache/map/maptrv2_teacher_train_spatial_gt20.pkl'),
    val=dict(
        type='NuScenes3DDataset',
        data_root='data/nuscenes/',
        classes=[
...
custom_hooks = [
    dict(
        type='MapKDCurriculumHook',
        schedules=dict(
            map_spatial_distill_alpha=dict(
                start_iter=10548,
                end_iter=15822,
                start_value=0.5,
                end_value=0.25)))
...
map_spatial_teacher_cache_path = 'data/cache/map/maptrv2_teacher_train_spatial_gt20.pkl'
gpu_ids = range(0, 2)
