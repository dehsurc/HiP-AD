...
work_dir = "work_dirs/exp/E14_stage1_det_spatial_a3"
...
wandb_project = "hipad"
wandb_name = "E14_stage1_det_spatial_a3"
log_config = dict(
    interval=50,
    hooks=[
...
# --- det distillation (BEVFusion teacher) ---
...
# --- map distillation (MapTRv2 teacher) ---
# feat_top20_l345 cache carries logits/pts/scores (same as the plain cache,
# top-20 queries) plus decoder point features for feature KD.
...
map_distill_mode = 'teacher_tp'
map_gt_loss_weight = 1.0

# --- task rebalancing (E13 recipe) ---
det_task_weight = 1.0
map_task_weight = 1.5

# --- map feature distillation (MapTRv2 decoder point features, E9 recipe) ---
...
# --- map spatial feature distillation (FPN-level, E9 recipe) ---
map_spatial_teacher_cache_path = 'data/cache/map/maptrv2_teacher_train_spatial_gt20.pkl'
map_spatial_distill_cfg = dict(
    map_spatial_distill_alpha=2.0,
...
# --- det spatial feature distillation (BEVFusion fused BEV features) ---
det_spatial_teacher_cache_path = 'data/cache/det/bevfusion_spatial_gt_p5.pkl'
det_spatial_distill_cfg = dict(
    det_spatial_distill_alpha=3.0,
    det_spatial_distill_teacher_dim=512,
    det_spatial_distill_num_levels=4,
    det_spatial_distill_class_balanced=True,
    det_spatial_detach_fpn_for_kd=False)
workflow = [("train", 1)]
...
model = dict(
    ...
    head=dict(
        ...
        onedecoder_head=dict(
            ...
            det_task_weight=det_task_weight,
            map_task_weight=map_task_weight,
            ...
            **map_feature_distill_cfg,
            **map_spatial_distill_cfg,
            **det_spatial_distill_cfg,
...
train_pipeline = [
    ...
    dict(
        ...
        keys=[
            ...
            "teacher_map_spatial_pts",
            "teacher_map_spatial_labels",
            "teacher_map_spatial_features",
            "teacher_det_spatial_pts",
            "teacher_det_spatial_labels",
            "teacher_det_spatial_features",
        ],
...
data = dict(
    ...
    train=dict(
        **data_basic_config,
        ...
        map_spatial_teacher_cache_path=map_spatial_teacher_cache_path,
        det_spatial_teacher_cache_path=det_spatial_teacher_cache_path,
    ),
    val=dict(
        **data_basic_config,
        ann_file=anno_root + "nuscenes_infos_val.pkl",
...
# E13 recipe: decay map spatial KD alpha 2.0 -> 1.0 over epochs 6..9 of 12.
custom_hooks = [
    dict(
        type="MapKDCurriculumHook",
        schedules=dict(
            map_spatial_distill_alpha=dict(
                start_iter=num_iters_per_epoch * 6,
                end_iter=num_iters_per_epoch * 9,
                start_value=2.0,
                end_value=1.0,
            )
        ),
