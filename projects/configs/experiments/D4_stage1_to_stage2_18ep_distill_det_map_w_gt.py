# D4 stage1 ckpt → stage2 18ep, KD on TOP of real GT for det + map (continued).
#
# Stage2 continuation of D4_stage1_12ep_distill_det_map_w_gt:
#   - real GT supervision stays ON for ALL tasks
#       (det_gt_loss_weight=1.0, map_gt_loss_weight=1.0)
#   - det KD (BEVFusion teacher) + map KD (MapTRv2 teacher) remain active in
#     pseudo_gt mode, identical hyperparameters to D4 stage1
#   - motion + planning + ego tasks come on-line in stage2 (use real GT only —
#     those tasks have no teacher cache)
#
# Differs from D2_stage1_to_stage2_18ep_kdonly.py only in:
#   - load_from points to the D4 stage1 ckpt
#   - det_gt_loss_weight=1.0 (vs 0.0)
#   - map_gt_loss_weight=1.0 (vs 0.0)
#
# Differs from D2_stage1_to_stage2_18ep.py (which dropped KD entirely in stage2)
# in that KD branches stay ON here.
...
wandb_name = "D4_stage1_to_stage2_18ep_distill_det_map_w_gt"
work_dir = "work_dirs/exp/D4_stage1_to_stage2_18ep_distill_det_map_w_gt"
...
# ------------------------------------------------------------------------
# Load D4 stage1 final ckpt (12 epoch = iter 21096, batch=8 × 2 GPUs)
# ------------------------------------------------------------------------
load_from = "/home/kyungmin/min_ws/rideflux/HiP-AD/work_dirs/exp/D4_stage1_12ep_distill_det_map_w_gt/iter_21096.pth"

# ------------------------------------------------------------------------
# Det KD (BEVFusion teacher) — same as D4 stage1; gt loss ON
# ------------------------------------------------------------------------
...
# ------------------------------------------------------------------------
# Map KD (MapTRv2 teacher) — same as D4 stage1; gt loss ON
# ------------------------------------------------------------------------
