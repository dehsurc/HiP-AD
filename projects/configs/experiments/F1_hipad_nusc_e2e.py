...
work_dir = "work_dirs/hipad_nusc_e2e"
...
# Mirrors VAD_base_e2e (= stage1 + stage2 epochs) — train all tasks from scratch.
# HiP-AD stage1 (12ep, det+map) + stage2 (18ep, +motion/plan/ego) = 30ep.
num_epochs = 60
...
wandb_project = "hipad"
wandb_name = "hipad_nusc_e2e_30ep"
log_config = dict(
    interval=50,
    hooks=[
...
# Single-stage end-to-end training — no stage1 checkpoint, ImageNet-pretrained backbone only.
load_from = None
resume_from = None
workflow = [("train", 1)]
...
# Enable every task from iter 0; motion is gated by task_select (sparse_onedecoder.py:341/952/1135).
...
model = dict(
    ...
    head=dict(
        ...
        onedecoder_head=dict(
            ...
            # loss — full stage2 weights (HiP-AD doesn't down-weight perception in stage2, so no
            # "stage1 weights vs stage2 weights" tradeoff like VAD; one weight set is correct).
...
# ================== training ========================
# CosineAnnealing over the full 30 epochs gives a single LR schedule for all tasks;
# stage1+stage2 used two separate schedules (12ep + 18ep).
optimizer = dict(
    type="AdamW",
    lr=1e-4,
    weight_decay=0.001,
