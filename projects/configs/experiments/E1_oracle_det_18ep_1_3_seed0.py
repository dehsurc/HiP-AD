"""Scenario A — det-oracle sanity check.

Goal: measure planning ceiling if det were perfect. Replaces det module output
with GT boxes/labels at the motion-query / inter_gnn slot. Map / motion / plan
/ ego train as usual. det modules are frozen and their loss is zero-weighted so
they don't contribute to the optimizer.

Based on PM_full_18ep_1_3_seed0.py — the same data split and schedule. Compare
planning metrics here vs. PM_full to read off how much of the planning gap is
...
work_dir = "work_dirs/exp/E1_oracle_det_18ep_1_3_seed0"
...
wandb_project = "hipad"
wandb_name = "E1_oracle_det_18ep_1_3_seed0"
log_config = dict(
    interval=50,
    hooks=[
...
import os as _os
# No det/map init needed — det is GT, map trains from scratch.
load_from = None
_resume = "./work_dirs/exp/E1_oracle_det_18ep_1_3_seed0/latest.pth"
...
temporal = True
# Oracle det is per-frame GT — no point carrying det features across frames.
temporal_det = False
temporal_map = True
temporal_ego = True
temporal_plan = True
...
model = dict(
    ...
    head=dict(
        ...
        onedecoder_head=dict(
            ...
            # ===== Oracle perception (Scenario A) =====
            oracle_det=True,
            oracle_det_freeze=True,
            # det_gt_loss_weight=0 also zeros the (discarded) det losses below,
            # so they don't affect the optimizer.
            ...
            temp_graph_model=dict(
                ...
                attn=[
                    dict(type="MultiheadFlashAttention", embed_dims=embed_dims * 2, num_heads=num_groups,
                         batch_first=True, dropout=drop_out),
                    dict(type="MultiheadFlashAttention", embed_dims=embed_dims, num_heads=num_groups,
                         batch_first=True, dropout=drop_out),
                    dict(type="MultiheadFlashAttention", embed_dims=embed_dims, num_heads=num_groups,
                         batch_first=True, dropout=drop_out),
                ],
            ...
            det_deformable=dict(
                ...
                attn_drop=0.15, use_deformable_func=use_deformable_func, use_camera_embed=True,
                residual_mode="cat",
                kps_generator=dict(
                    type="SparseBox3DKeyPointsGenerator",
                    num_learnable_pts=6,
                    fix_scale=[[0, 0, 0], [0.45, 0, 0], [-0.45, 0, 0], [0, 0.45, 0], [0, -0.45, 0],
                               [0, 0, 0.45], [0, 0, -0.45]],
                ),
            ),
            map_deformable=dict(
                ...
                kps_generator=dict(
                    ...
                    embed_dims=embed_dims, num_sample=map_num_pts, num_learnable_pts=3,
                    fix_height=(0, 0.5, -0.5, 1, -1), ground_height=-1.84023,
                ),
            ),
            ego_deformable=dict(
                type="DeformableFeatureAggregation",
            ...
            plan_deformable=dict(
                ...
                kps_generator=dict(
                    type="SparsePoint3DKeyPointsGenerator",
                    embed_dims=embed_dims, num_sample=ego_fut_ts, num_learnable_pts=3,
                    fix_height=(0, 0.5, -0.5, 1, -1), ground_height=-1.84023,
                ),
            ),
            ...
            plan_refine_layer=dict(
                ...
                embed_dims=embed_dims, ego_fut_ts=ego_fut_ts, ego_fut_cmd=ego_fut_cmd,
                ego_fut_mode=ego_fut_mode, anchor_types=plan_anchor_types,
            ),
            ...
            # det losses are zero-weighted via det_gt_loss_weight above; the
            # raw weights here are kept so loss_det can still build and run
            # det_sampler.indices (needed by motion_sampler).
...
# Test pipeline must surface gt_bboxes_3d / gt_labels_3d because the model
# uses them at inference time (oracle injection runs in both train and eval).
test_pipeline = [
    dict(type="LoadMultiViewImageFromFiles", to_float32=True),
    dict(type="ResizeCropFlipImage"),
    dict(type="NormalizeMultiviewImage", **img_norm_cfg),
...
# det/motion metrics are meaningless under oracle (det=GT, motion conditioned
# on GT). Keep planning + map for the actual reading.
eval_mode = dict(
    with_det=False,
    with_tracking=False,
    with_map=True,
