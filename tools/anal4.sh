#!/usr/bin/env bash
# GPU 3 / ckpt 18ep / num_batches=2000 (b2000 yaml).
...
CUDA_VISIBLE_DEVICES=3 PYTORCH_CUDA_ALLOC_CONF=max_split_size_mb:512 \
  PYTHONPATH=. /home/yongjae/miniconda3/envs/hipad/bin/python tools/run_gradient_analysis.py \
  --config configs/gradient_analysis_b2000.yaml --checkpoints 18ep \
  ...
  --no-supplementary \
  2>&1 | tee gradient_analysis_results/inter_gnn_b1000_pcgrad_18ep.log
