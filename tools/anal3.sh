#!/usr/bin/env bash
# GPU 2 / ckpt 9ep / num_batches=2000 (b2000 yaml).
...
CUDA_VISIBLE_DEVICES=2 PYTORCH_CUDA_ALLOC_CONF=max_split_size_mb:512 \
  ...
  --config configs/gradient_analysis_b2000.yaml --checkpoints 9ep \
  ...
  --no-supplementary \
  2>&1 | tee gradient_analysis_results/inter_gnn_b1000_pcgrad_9ep.log
