#!/usr/bin/env python
"""
Gradient Conflict Analysis Script for HiP-AD Multi-Task Learning (nuScenes version)
...
Adapted from gradient_conflict_anal.py (Bench2Drive version) for nuScenes dataset.

Key differences from Bench2Drive version:
  - Uses mmdet's build_dataset / build_dataloader (not custom_build_dataset)
  - Supports both nuScenes loss key patterns: 'det_loss_*' and 'loss_det_*'
  - FP16 via wrap_fp16_model (nuScenes config uses fp16 dict)
  - Data handling via scatter_kwargs + forward_train (nuScenes DataContainer format)
  - Includes 'depth' task (loss_dense_depth)
...
Usage:
    python projects/gradient_conflict_anal_nusc.py \
        --config projects/configs/hipad_nusc_stage2.py \
        --checkpoint work_dirs/hipad_nusc_stage2/latest.pth \
        --num-batches 50 \
        --batch-size 1 \
        --analysis-mode full \
        --output-dir gradient_analysis_results_nusc
"""
...
# nuScenes supports both loss key patterns: 'det_loss_*' and 'loss_det_*'
TASK_GROUPS = {
    'det': ['det_loss_cls', 'det_loss_box', 'det_loss_cns', 'det_loss_yns',
            'loss_det_cls', 'loss_det_reg'],
    'map': ['map_loss_cls', 'map_loss_line',
            'loss_map_cls', 'loss_map_reg'],
    'motion': ['motion_loss_cls', 'motion_loss_reg',
               'loss_motion_cls', 'loss_motion_reg'],
